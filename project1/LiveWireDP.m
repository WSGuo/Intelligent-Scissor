...
for row = 1:(height/3)
    for col = 1: (width/3)
        ...
        posRow = 3*(row-1) + 2;
        posCol = 3*(cal-1) + 2;
        cost0 = costGraph(posRow,posCol+1,0);
        cost1 = costGraph(posRow-1,posCol+1,0);
        cost2 = costGraph(posRow-1,posCol,0);
        cost3 = costGraph(posRow-1,posCol-1,0);
        cost4 = costGraph(posRow,posCol-1,0);
        cost5 = costGraph(posRow+1,posCol-1,0);
        cost6 = costGraph(posRow+1,posCol,0);
        cost7 = costGraph(posRow+1,posCol+1,0);
