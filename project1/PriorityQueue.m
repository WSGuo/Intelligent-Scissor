...
classdef PriorityQueue < handle
    ...
    properties (SetAccess = private)               
       ...
       priorityList;
       valueList;
       flagMinPriorityQueue;
    end

    methods (Access = public)       

        function obj = PriorityQueue( flagMinPriorityQueue )

            if ~exist( 'flagMinPriorityQueue', 'var' )
                flagMinPriorityQueue = true;
            else
                if ~(isscalar(flagMinPriorityQueue) && islogical(flagMinPriorityQueue))
                    error( 'ERROR: invalid flagMinPriorityQueue argument' );
                end
            end

            obj.flagMinPriorityQueue = flagMinPriorityQueue;
            obj.numElements = 0;
            obj.priorityList = {};
            obj.valueList = {};
        ...
        function [priority, value] = peek( obj )
            ...
            if obj.isEmpty()
                error( 'requested min() of an empty priority queue' );
            end          
